...
> **Важная деталь**: По умолчанию и nginx, и multitool хотят использовать порт 80, что создаст конфликт. Поэтому для multitool настраиваем порт 8080 через переменные окружения `HTTP_PORT` и `HTTPS_PORT`.
...
> **Результат**: Service успешно балансирует трафик между двумя репликами. Оба контейнера (nginx на порту 80 и multitool на порту 8080) доступны через единую точку входа.
...
- Решен конфликт портов через переменные окружения
...
- Init-контейнер блокирует запуск nginx до появления Service
